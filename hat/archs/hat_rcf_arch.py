...
# Import RCF from the previously created file
from .rcf_arch import RCF

# --- All the helper classes from hat_arch.py (DropPath, CAB, Mlp, etc.) should be here --- #
# For brevity, assuming they are copied here. I will copy the essential ones.
...
class RHAG(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, compress_ratio, squeeze_factor, conv_scale, overlap_ratio, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, img_size=224, patch_size=4, resi_connection='1conv'):
        ...
        self.residual_group = AttenBlocks(
            ...
            compress_ratio=compress_ratio, squeeze_factor=squeeze_factor, conv_scale=conv_scale,
            overlap_ratio=overlap_ratio, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop,
            attn_drop=attn_drop, drop_path=drop_path, norm_layer=norm_layer, downsample=downsample, use_checkpoint=use_checkpoint)
...
class HAT_RCF(nn.Module):
    def __init__(self,
        ...
        if in_chans == 3:
            hat_rgb_mean = (0.4488, 0.4371, 0.4040)
            self.mean = torch.Tensor(hat_rgb_mean).view(1, 3, 1, 1)
        else:
        ...
        rcf_rgb_mean = (104.00698793, 116.66876762, 122.67891434)
        self.rcf_mean = torch.Tensor(rcf_rgb_mean).view(1, 3, 1, 1)

        self.rcf = RCF(pretrained=rcf_pretrained_path)
        # Freeze RCF parameters
        for param in self.rcf.parameters():
        ...
        self.edge_downsampler = nn.Sequential(
            ...
            # --- Block 2: Feature Extraction at Half Resolution ---
    ...
    def forward(self, x):
        ...
        self.rcf_mean = self.rcf_mean.type_as(x)

        # RCF branch
        with torch.no_grad():
            # The input tensor x is in range [0, 1] with 3 channels (RGB).
            # RCF expects BGR format with pixel values in [0, 255] before mean subtraction.
            x_rcf_input = x.flip(1) # RGB to BGR
            x_rcf_input = x_rcf_input * 255.0
            x_rcf_input = x_rcf_input - self.rcf_mean
        ...
        # HAT branch
        # HAT expects input in range [0, 1] and subtracts its own mean.
        ...
        refined_fused_features = self.conv_after_body(fused_features)

        # Residual connection and upsampling
        res = refined_fused_features + x_first

        if self.upsampler == 'pixelshuffle':
            res = self.conv_before_upsample(res)
            res = self.conv_last(self.upsample(res))
        else:
            # Fallback for other upsamplers, though pixelshuffle is standard for HAT
